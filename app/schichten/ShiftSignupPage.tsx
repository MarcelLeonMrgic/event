...
import { ShiftPlan } from "@/lib/shifts";
...
type ShiftSignupPageProps = {
  plan: ShiftPlan;
};

export default function ShiftSignupPage({ plan }: ShiftSignupPageProps) {
  const [selectedShift, setSelectedShift] = useState(plan.shifts[0].id);
  ...
  useEffect(() => {
    async function loadSignups() {
      try {
        const response = await fetch(`/api/signups?location=${plan.key}`);
  ...
  const activeShift = useMemo(
    () =>
      plan.shifts.find((shift) => shift.id === selectedShift) ??
      plan.shifts[0],
    [plan.shifts, selectedShift],
  );
    ...
    <main className="min-h-screen bg-[#f7f3ed] text-[#171512]">
      <nav className="mx-auto flex w-full max-w-6xl items-center justify-between px-6 py-6">
        ...
        <div className="hidden items-center gap-8 text-sm font-medium text-[#5f5a51] sm:flex">
          ...
          <Link
            className={plan.key === "makerspace" ? "text-[#171512]" : "transition hover:text-[#171512]"}
            href="/schichten"
          >
            Makerspace
          </Link>
          <Link
            className={plan.key === "zwille" ? "text-[#171512]" : "transition hover:text-[#171512]"}
